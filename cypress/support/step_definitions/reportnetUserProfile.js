...
And("I can change the visual {string} from {string} to {string}",(option, oldFormat, newFormat) => {   
    if(option === 'theme') {
        cy.get('[class*=InputSwitch_inputswitch_dark_theme]:last').click({force:true}) 
    } 
    else {
...
And("I can go to the dataflows page",() => {
    cy.wait(1000)
    cy.get('.p-menuitem-link:first').click({force:true})
})
...
And("I can see the theme is {string}", (theme) => {
    cy.wait(1000)
    const css = theme === "Light" ? '--bg:var(--white)' : '--bg:var(--c-dark-blue)'
    cy.get('body').should('have.attr', 'style').and('contains', css)
})

And("I can add a user {string}", (fileName) => {
    cy.get('[class*=UserImg_userDataIcon]').click({force:true})
    if(fileName === "") {
        cy.get(".p-button-text:contains(Select one)").click({force:true})
        cy.get('[class*=UserImg_gridItem]:visible:last').click({force:true})
    } else {
